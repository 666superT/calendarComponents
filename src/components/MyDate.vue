    ...
    <div class="content">
      <div :style="{ top: positionTop + 'px' }" class="days">
        <div class="item rowJcAc" v-for="(item, index) in dates" :key="index">
          ...
          >
            <div
              v-if="isMarkDay(item.year, item.month, item.date).display"
              class="markDay"
              :class="[
                choose == `${item.year}-${item.month}-${item.date}`
                  ? 'markDayChoose'
                  : isMarkDay(item.year, item.month, item.date).markClass,
              ]"
            ></div>
            {{ Number(item.date) }}
          </div>
...
export default {
  ...
  methods: {
    ...
    isMarkDay(y, m, d) {
      ...
      for (let i = 0; i < this.markDays.length; i++) {
        ...
        if (this.markDays[i].date == dy) {
          flag = true;
          markClass =
            this.markDays[i].type == "yes" ? "markDayCheck" : "markDayLate";
          break;
        }
      }
      return {
    ...
    // 点击回调
    selectOne(i) {
      let markDay = this.isMarkDay(i.year, i.month, i.date).display;
